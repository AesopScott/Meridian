...
def test_workflow_logic_snapshot_documents_subagent_harness():
    ...
    assert "Workflow owns bounded work-order execution shape" in snapshot["summary"]
...
def test_index_workflow_harness_uses_backend_logic_snapshot():
    doc = (ROOT / "index.html").read_text(encoding="utf-8")
    assert "Workflow Runtime Logic" in doc
    assert "data-workflow-logic" in doc
    assert "bridgeUrl('workflow-logic')" in doc
    assert "renderWorkflowLogicSnapshot" in doc
    assert "renderWorkflowSubagentLogic" in doc
    assert "button.dataset.harness === 'Workflow'" in doc
...
def test_bridge_exposes_workflow_logic_route_and_capability():
    doc = (ROOT / "scripts" / "meridian-model-bridge.js").read_text(encoding="utf-8")
    assert "workflowLogicSnapshot: true" in doc
    assert "workflowLogic: '/bridge/workflow-logic'" in doc
    assert "meridian_core.workflow_logic_snapshot" in doc
    assert "req.url === BRIDGE_ROUTES.workflowLogic" in doc
