...
class TestExecuteRelayDispatchPlanWithPolicy:
    ...
    def test_prompt_packet_policy_evidence_attached_to_decision_record(self):
        plan = _make_plan(2)
        summary = execute_relay_dispatch_plan_with_policy(
            plan,
            _constant_model_call("ok"),
            proof_trail=_clean_proof_trail("packet-proof-decision"),
            include_decision_record=True,
        )

        evidence = summary.decision_record.prompt_packet_policy_evidence
        assert isinstance(evidence, RelayPromptPacketPolicyEvidence)
        assert evidence.decision == "allow"
        assert evidence.evidence_ids == ("packet-proof-decision",)
        assert evidence.packet_hash == plan.packet.proof_metadata.packet_hash

    def test_prompt_packet_policy_warn_degraded_evidence_is_display_safe(self):
        plan = _make_plan(1)
    ...
    def test_prompt_packet_policy_missing_proof_metadata_fails_closed(self):
        ...
        packet = dataclasses.replace(
            plan.packet,
            proof_metadata=False,  # type: ignore[arg-type]
        )
        plan_without_metadata = dataclasses.replace(plan, packet=packet)
