...
function echoLogicSnapshot() {
  return new Promise((resolve) => {
    const pythonBin = process.env.MERIDIAN_PYTHON_BIN || 'python';
    const child = spawn(pythonBin, ['-m', 'meridian_core.echo_logic_snapshot'], {
      cwd: DEFAULT_CWD,
      shell: process.platform === 'win32',
      windowsHide: true,
      env: {
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        resolve({ ok: false, error: stderr.trim() || `Echo logic snapshot exited with code ${code}` });
        return;
      }
      try {
        resolve(JSON.parse(stdout));
      } catch (error) {
        resolve({ ok: false, error: `Echo logic snapshot returned invalid JSON: ${error.message}` });
      }
    });
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === BRIDGE_ROUTES.echoLogic) {
    const snapshot = await echoLogicSnapshot();
    sendJson(res, snapshot.ok ? 200 : 500, {
      service: 'meridian-model-bridge',
      version: BRIDGE_VERSION,
      capabilities: BRIDGE_CAPABILITIES,
