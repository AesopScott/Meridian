...
const BRIDGE_ROUTES = Object.freeze({
  ...
  federationLogic: '/bridge/federation-logic',
...
function federationLogicSnapshot() {
  return new Promise((resolve) => {
    const pythonBin = process.env.MERIDIAN_PYTHON_BIN || 'python';
    const child = spawn(pythonBin, ['-m', 'meridian_core.federation_logic_snapshot'], {
      cwd: DEFAULT_CWD,
      shell: process.platform === 'win32',
      windowsHide: true,
      env: {
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        resolve({ ok: false, error: stderr.trim() || `Federation logic snapshot exited with code ${code}` });
        return;
      }
      try {
        resolve({ ok: true, snapshot: JSON.parse(stdout) });
      } catch (error) {
        resolve({ ok: false, error: `Federation logic snapshot returned invalid JSON: ${error.message}` });
      }
    });
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === BRIDGE_ROUTES.federationLogic) {
    const result = await federationLogicSnapshot();
    sendJson(res, result.ok ? 200 : 500, {
      ok: result.ok,
      service: 'meridian-model-bridge',
      version: BRIDGE_VERSION,
