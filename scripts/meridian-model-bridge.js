...
function atlasLogicSnapshot() {
  return new Promise((resolve) => {
    const pythonBin = process.env.MERIDIAN_PYTHON_BIN || 'python';
    const child = spawn(pythonBin, ['-m', 'meridian_core.atlas_logic_snapshot'], {
      cwd: DEFAULT_CWD,
      shell: process.platform === 'win32',
      windowsHide: true,
      env: {
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        resolve({ ok: false, error: stderr.trim() || `Atlas logic snapshot exited with code ${code}` });
        return;
      }
      try {
        resolve(JSON.parse(stdout));
      } catch (error) {
        resolve({ ok: false, error: `Atlas logic snapshot returned invalid JSON: ${error.message}` });
      }
    });
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === BRIDGE_ROUTES.atlasLogic) {
    const snapshot = await atlasLogicSnapshot();
    sendJson(res, snapshot.ok ? 200 : 500, {
      service: 'meridian-model-bridge',
      version: BRIDGE_VERSION,
      capabilities: BRIDGE_CAPABILITIES,
