...
function aegisLogicSnapshot() {
  return new Promise((resolve) => {
    const child = spawn('python', ['-m', 'meridian_core.aegis_logic_snapshot'], {
      cwd: DEFAULT_CWD,
      shell: process.platform === 'win32',
      windowsHide: true,
    });
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        resolve({ ok: false, error: stderr.trim() || `Aegis logic snapshot exited with code ${code}` });
        return;
      }
      try {
        resolve({ ok: true, snapshot: JSON.parse(stdout) });
      } catch (error) {
        resolve({ ok: false, error: `Aegis logic snapshot returned invalid JSON: ${error.message}` });
      }
    });
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === BRIDGE_ROUTES.aegisLogic) {
    const result = await aegisLogicSnapshot();
    sendJson(res, result.ok ? 200 : 500, {
      ok: result.ok,
      service: 'meridian-model-bridge',
      version: BRIDGE_VERSION,
