...
def _first_proof_requirement(proof_required: tuple[str, ...]) -> str:
    if not proof_required:
        return "none"
    first = proof_required[0]
...
def _build_aegis_prompt_packet_policy_metadata(
    ...
    *,
    ...
    proof_required = (
        dispatch_envelope.proof_required
        if dispatch_envelope and dispatch_envelope.proof_required
        else (packet_proof.proof_required if packet_proof else tuple(plan.route.audit.proof_required))
    )
    ...
    return AegisPromptPacketProofMetadata(
        ...
        proof_requirement=_first_proof_requirement(tuple(proof_required)),
        selected_model_id=dispatch_envelope.exact_model_id if dispatch_envelope else None,
        model_trust_state=(
            dispatch_envelope.trust_state if dispatch_envelope else plan.route.audit.trust_state.value
        ),
        ...
        dual_lane_proof_present=bool(evidence_ids),
    )
