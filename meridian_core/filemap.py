...
def make_default_map() -> FileMap:
    ...
    entries = [
        ...
        FileMapEntry(
            path="meridian_core/filemap_logic_snapshot.py",
            area=FileArea.FILE_MAP,
            purpose="Backend snapshot for Charon/FileMap Runtime Logic: entry totals, area coverage, required path registration, related-test visibility, memory injection behavior, and no-write boundary.",
            related_tests=["tests/test_filemap_logic_snapshot.py", "tests/test_bifrost_cockpit.py"],
            notes="Bifrost consumes this through /bridge/filemap-logic; keeps Charon UI backend-sourced and lookup-only.",
        ),
        FileMapEntry(
        ...
        FileMapEntry(
            path="tests/test_filemap_logic_snapshot.py",
            area=FileArea.FILE_MAP,
            purpose="Test suite for meridian_core/filemap_logic_snapshot.py: proves Charon Runtime Logic derives from make_default_map and remains lookup-only.",
            related_tests=[],
            notes="Run before changing Charon snapshot fields, bridge exposure, or harness logic rendering.",
        ),
