...
def make_default_map() -> FileMap:
    ...
    entries = [
        ...
        FileMapEntry(
            ...
            area=FileArea.BEACON,
            purpose="Backend snapshot for Beacon heartbeat, liveness, freshness, advisory evidence, and runtime boundary logic shown in the visible harness panel.",
            related_tests=["tests/test_beacon_logic_snapshot.py", "tests/test_bifrost_cockpit.py"],
            notes="Bifrost consumes this through /bridge/beacon-logic; keeps Beacon UI observation-only and backend-sourced.",
        ),
        FileMapEntry(
        ...
        FileMapEntry(
            ...
            area=FileArea.BEACON,
            purpose="Test suite for meridian_core/beacon_logic_snapshot.py: proves Beacon Runtime Logic stays display-safe, source-identified, and observation-only.",
            related_tests=[],
            notes="Run before changing Beacon snapshot fields, bridge exposure, or harness logic rendering.",
        ),
        FileMapEntry(
        ...
        FileMapEntry(
            ...
            area=FileArea.BIFROST,
            purpose="Design brief for how the Meridian cockpit displays queue-driven worker activity â€” display, not activation. What user sees and how build-lane events surface without flooding the cockpit.",
            related_tests=[],
        ...
        FileMapEntry(
            ...
            area=FileArea.BIFROST,
            purpose="Harness dashboard surface brief: what opens when user clicks the Harness button â€” observability of every harness (heartbeat, capabilities, maturity, recent events). Observation-first; no controls in V0.",
            related_tests=[],
