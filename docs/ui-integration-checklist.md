...
| MB10 | Bridge restart endpoint | Local bridge exposes a same-port restart endpoint for Reset recovery. | `POST /bridge/restart` returns accepted, duplicate in-flight restart calls are idempotent, child processes are hidden on Windows, then `/bridge/health` and `/bridge/models` come back with visible-context capability. |
