...
| SKL10 | Run/request path | Provides a clear path to invoke or request the skill when supported. | wired | Skills Registry shows non-executing usage paths such as opening a relative path through FileMap/Atlas or selecting an already available backend in the manual model selector; the surface itself does not run skills. |
