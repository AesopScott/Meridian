...
- Repair commits: `95dde4d50` (Repair Compass scope-layer subject_ref/ambiguity_reason raw-context leak) and `43b1dafb` (test: cover scope raw subject precedence redaction).
- Files changed: `meridian_core/compass.py` (+22 lines, raw-context guard at the top of `evaluate_project_scope` now builds BLOCKED `ProjectScopeEvaluation` directly with `subject_ref` and `evidence_refs` redacted), `tests/test_compass.py` (+184 lines: new `TestProjectScopeSubjectFieldRawContextGuard` class with 20 cases, plus the follow-up combined raw-evidence/raw-subject precedence assertion), `docs/live-build-4.md` (this branch marker).
...
- Concrete evidence each required invariant is enforced:
  ...
  - Precedence: `test_raw_context_in_both_subject_ref_and_evidence_refs_blocks` documents that evidence_refs guard fires first while still redacting `subject_ref`; the test asserts both raw evidence and raw subject payloads are absent from JSON, then retries with safe evidence to prove the subject-field guard also fires directly.
...
- Next Candidate: pending coordinator promotion after this scope-layer subject-field redaction repair clears Codex review. Independent Codex review still owed for `cc584318f` + `cd20be9c3` + `270438271` + `df8120b49` + `808297315` + `95dde4d50` + `43b1dafb`.
...
- Files changed: `meridian_core/compass.py` (+77 lines total), `tests/test_compass.py` (+277 lines total), `docs/live-build-4.md` (this branch marker).
- Tests run: `python -m pytest tests/test_compass.py -q` -> **248 passed** (37 + 2 repair tests = 39 new + 1 updated existing test on top of 208 prior).
- `git diff --check`: clean.
...
- Concrete evidence each required invariant is enforced:
  ...
  - Self-review repair `df8120b49`: shared_relationship_refs ALSO redacted on the raw-context BLOCKED branch. The prior implementation computed `_shared_relationship_refs(left, right)` before the raw-context guard and preserved any raw payload that both sides smuggled through repo_refs/venture_refs (e.g. `repo_refs=("raw_prompt:smuggled",)` on both sides). `test_shared_relationship_refs_redacted_when_raw_context_shared` and `test_shared_relationship_refs_redacted_when_raw_context_shared_via_venture` regression-guard the fix; raw payload absent from `json.dumps` output across both repo_refs and venture_refs leak paths.
  - Defense-in-depth: raw-context payload smuggled through profile fields is also caught. Left- and right-side `mission_bearing` each blocks via dedicated tests. Both sides parametrized over `objectives`, `artifacts`, `memory_pins`, `blockers`, `proof_expectations`, `repo_refs`, `venture_refs` (14 tests total) each block with side+field-specific blocker name and no raw payload in JSON.
  ...
  - Multi-site aggregation: `test_multiple_raw_context_sites_aggregate_into_blockers` confirms all raw-context sites surface in the blockers tuple in one pass.
...
- Push status: branch-local commit only this turn per coordinator directive ("do not push"). No `docs/live-build-4.md` update on shared main this turn; the slice can be promoted to main on a subsequent coordinator-authorized heartbeat.
