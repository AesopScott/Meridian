...
| `index.html` | Bifrost / session harness | Browser-hosted Meridian cockpit shell: renders the harness dock, session panels, model bridge status, and Relay model logic surface. | `tests/test_relay_logic_snapshot.py` | Root static UI. Reads local bridge endpoints such as `/api/models` and `/api/relay-logic`; keep UI behavior in sync with `scripts/meridian-model-bridge.js`. |
| `scripts/meridian-model-bridge.js` | Bifrost / session harness | Local Node model bridge for the browser cockpit: exposes model message, recent-call, restart, health, and Relay logic snapshot endpoints. | `tests/test_relay_logic_snapshot.py` | Development bridge on `127.0.0.1:8767`. Spawns `meridian_core.relay_logic_snapshot` for `/api/relay-logic`. |
