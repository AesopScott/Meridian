...
| `meridian_core/prompt_budget.py` | Relay prompt budget | Deterministic prompt token budget per risk tier. Prevents Relay prompt drag by bounding context sources and token limits per dispatch. | `tests/test_prompt_budget.py` | Tiers 0–1 minimal; Tier 2 focused; Tier 3 bounded proof/review; Tier 4 explained for human gate. |
