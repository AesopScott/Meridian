...
- The Meridian UI is the Electron app. Root `index.html` is renderer internals
  loaded by that app, not the UI identity and not a separate browser demo
  target. Deterministic Bifrost preview HTML exists only for
